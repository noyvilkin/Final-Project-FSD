...
describe('ZipProcessor.scanZipFile', () => {
  const PACKAGES_BASE = path.join(__dirname, '..', '..', '..', 'features', 'assignment', 'tests', 'faulty-packages');
  ...
  test('filters noise and returns only source files for package-01', async () => {
    ...
    // No noise paths should be present
    const nonSource = result.sourceFiles.filter(f => /node_modules|\.git|dist|build|\.idea|\.vscode/i.test(f.path));
    expect(nonSource.length).toBe(0);

    // Languages should be detected for source files
